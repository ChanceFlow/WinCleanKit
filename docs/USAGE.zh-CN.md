...
双击 `run.bat`，或运行 `src\WinCleanKit.bat`。两者都会先提权。

`run.bat` 请求的是**窗口界面**；单独运行 `src\WinCleanKit.bat` 打开的是全屏控制台界面 ——
也就是没有桌面可开窗口时跑的那个。每个界面都会报告自己能否启动，所以启动器是逐级降级而不是直接失败：
窗口 → 全屏控制台 → 编号菜单。
...
| `--lang=zh` / `--lang=en` | 同上。 |
| `--zh` / `--en` | 同上，短写法。 |
| `--gui` | 窗口界面。`run.bat` 会自动带上。 |
| `--tui` | 强制使用全屏控制台界面，即使这台机器能开窗口。 |
| `--simple` / `--no-tui` | 用数字菜单代替上面两种界面，适合自动化与读屏软件。 |
...
| `-Gui` | 打开窗口界面。在 `src\WinCleanKit.bat` 里对应 `--gui`，`run.bat` 会自动带上。 |
| `-Tui` | 打开全屏控制台界面。在 `src\WinCleanKit.bat` 里对应 `--tui`。 |
| `-TuiLanguage en\|zh\|ask` | 两种交互界面启动时使用的语言。`ask`（默认）会先显示选择屏。 |
