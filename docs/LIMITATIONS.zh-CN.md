...
`tests/Test-TuiRender.ps1` 比对的是"一帧由哪些行拼成"，这对这些行写进控制台之后会发生什么一无所知。两个真实缺陷就住在这个缝里：每行末尾多写了一个换行，于是每次重绘整屏下滚一行；语言切换改了状态，而按键循环判定"什么都没变"，于是按 `l` 之后旧的一帧留在屏幕上。

这两个都不是看出来的，而是在真实会话里驱动已部署的构建、再把控制台读回来发现的：

1. 在交互式会话里启动界面（SSH 会话属于 session 0，没有可绘制的控制台）。
2. 在同一会话里 `AttachConsole` 到引擎的 PowerShell 进程，用 `ReadConsoleOutputCharacterW` 读出窗口内容 —— 这样就能逐字节对比按键前后的缓冲区，也正是它把"这帧看着对"变成了"按 `l` 之后这帧一模一样"。
3. 用 `SendKeys`/`SendInput` 向前台控制台窗口发送按键，逐屏走查。

其中只有 `tools/Test-TuiScroll.ps1` 是提交进仓库、可重复执行的那一半：它绘制、重绘、把缓冲区读回来，一旦发生滚动就失败，并把旧画笔当作对照组，所以"通过"不可能是由一个检测不到任何东西的测试给出的。驱动按键的那一半是手工流程，就记在这里。
