...
| `presets` | 取自 `conservative`、`balanced`、`aggressive` 的数组。 |
...
3. 每个动作都有两种语言，且至少属于一个预设。
4. `conservative ⊆ balanced ⊆ aggressive`。加进小预设的动作，必须也在所有更大的预设里。
5. 没有动作针对 `hosts` 文件。
...
- **`-Only` 是权威列表。** 一旦出现，预设就只是标签。把它改成并集会破坏界面上的逐项取消勾选。
...
- 如果你的改动改变了某个预设的行为，请**明确指出** —— 这会影响到从不看 changelog 的用户。
