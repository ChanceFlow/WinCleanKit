...
.DESCRIPTION
    ...
    cursor movement and clamping, mode switching, preset resolution, selection
...
Check 'preset honoured' ($st.Preset -eq 'balanced')
Check 'selection matches the preset' ($st.Selected.Count -eq @($cat.actions | Where-Object { $_.presets -contains 'balanced' }).Count) ("{0} selected" -f $st.Selected.Count)
Check 'starts in list mode' ($st.Mode -eq 'list')
$stZh = Initialize-TuiState -Catalog $cat -Preset 'balanced' -Language 'zh'
...
$enNames = @((Initialize-TuiState -Catalog $cat -Preset 'balanced' -Language 'en').Groups | ForEach-Object { $_.name })
Check 'en names differ from zh names' (($enNames -join '|') -ne ($namesZh -join '|'))

# ---------------------------------------------------------------------------
Write-Head 'navigation clamps instead of wrapping'
$st = Initialize-TuiState -Catalog $cat -Preset 'balanced'
...
# ---------------------------------------------------------------------------
Write-Head 'presets'
$st = Initialize-TuiState -Catalog $cat -Preset 'conservative'
$nCons = $st.Selected.Count
$st = Select-TuiPreset -State $st -Preset 'aggressive'
$nAgg = $st.Selected.Count
$st = Select-TuiPreset -State $st -Preset 'conservative'
$nBack = $st.Selected.Count
Check 'preset grows and shrinks' ($nCons -lt $nAgg) ("conservative {0} < aggressive {1}" -f $nCons, $nAgg)
Check 'switching back restores the count' ($nBack -eq $nCons)
Check 'preset replaces rather than adds' ($nBack -eq @($cat.actions | Where-Object { $_.presets -contains 'conservative' }).Count)
$before = $st.Selected.Count
$st = Select-TuiPreset -State $st -Preset 'no-such-preset'
Check 'unknown preset is ignored' ($st.Selected.Count -eq $before -and $st.Preset -eq 'conservative')
Check 'preset order is safest first' (((Get-PresetList) -join ',') -eq 'conservative,balanced,aggressive')
Check 'rank lookup works' ((Get-PresetRank 'aggressive') -eq 2 -and (Get-PresetRank 'nope') -eq -1)
...
$st = Initialize-TuiState -Catalog $cat -Preset 'conservative'
$rCons = Get-TuiHighestRisk $st
$st = Select-TuiPreset -State $st -Preset 'aggressive'
$rAgg = Get-TuiHighestRisk $st
Check 'conservative risk <= aggressive risk' ($rCons -le $rAgg) ("{0} <= {1}" -f $rCons, $rAgg)
Check 'aggressive reaches high' ($rAgg -eq 3)
$st = Switch-TuiAllSelection -State $st -Selected $false
...
$st = Initialize-TuiState -Catalog $cat -Preset 'conservative'
$plan = Get-TuiPlan $st
$expected = @($cat.actions | Where-Object { $_.presets -contains 'conservative' } | ForEach-Object { $_.id })
Check 'plan ids match the preset' (($plan -join ',') -eq ($expected -join ',')) ("{0} ids" -f $plan.Count)
...
$st = Initialize-TuiState -Catalog $cat -Preset 'aggressive'
$st.DetailIndex = 300
